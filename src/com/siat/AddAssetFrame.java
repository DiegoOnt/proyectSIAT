...
public class AddAssetFrame extends JFrame {
    ...
    public AddAssetFrame(Usuario user, AssetUpdateListener listener) {
        this.currentUser = user;
        this.updateListener = listener;
        setTitle("Agregar Activo");
        setSize(520, 480);
        ...
        // Pre-fill SKU with "ACT-" prefix
        skuField.setText("ACT-");
        ...
        ModernButton save = new ModernButton("Guardar");
    ...
    private void onSave() {
        try {
            ...
            if (idx < 0) { messageLabel.setText("Seleccione una categoría"); return; }
            a.setCategory(categories.get(idx)[0]); // store category_id as string
            
            int locIdx = locationBox.getSelectedIndex();
            if (locIdx < 0) { messageLabel.setText("Seleccione una ubicación"); return; }
            ...
            if (a.getSku() == null || a.getSku().isBlank()) {
                messageLabel.setText("SKU es obligatorio.");
                return;
            }
            if (ActivoDAO.existsBySku(a.getSku())) {
                messageLabel.setText("Error: SKU ya existe en la base de datos.");
                return;
            }

            boolean ok = ActivoDAO.insert(a, currentUser.getUserId());
            if (ok) {
                com.siat.dao.AuditDAO.log(currentUser.getUserId(), "CREAR_ACTIVO", "assets", a.getSku(), true, "Activo creado: " + a.getName() + " (SKU: " + a.getSku() + ")");
                JOptionPane.showMessageDialog(this, "Activo creado correctamente.");
                if (updateListener != null) updateListener.onAssetSaved();
                dispose();
            } else {
                com.siat.dao.AuditDAO.log(currentUser.getUserId(), "CREAR_ACTIVO", "assets", a.getSku(), false, "Fallo al crear activo: " + a.getName() + " (SKU: " + a.getSku() + ")");
                messageLabel.setText("Error al crear activo (revisar logs)");
            }
